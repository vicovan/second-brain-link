...
`owner:` in your own traveller profile (47-travel/profile/traveler.md).
...
def owner_name():
    ...
    """
    try:
        from paths import profile_dir, work_root, other_profile_dir, LAYER
        for cand in (profile_dir(), other_profile_dir(), work_root() / LAYER / "profile"):
            if not cand:
                continue
            f = pathlib.Path(cand) / "traveler.md"
            if f.is_file():
...
def usable(p):
    from paths import layer
    # a brain with no places layer has nothing to plan a trip from
    return p and is_brain(p) and (pathlib.Path(p) / layer(p, "places")).is_dir()
